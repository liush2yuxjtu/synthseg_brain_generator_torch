...
def build_model_inputs(path_label_maps,
    ...
    mini-batch from the same distribution.
    ...
    :param return_tensors: (optional) whether to return PyTorch tensors instead of numpy arrays. Default is False.
    :param device: (optional) PyTorch device to use for tensors if return_tensors is True. Default is None.
    """

    # Set device if returning tensors
    if return_tensors and device is None:
    ...
    while True:
        ...
        # build dictionary of inputs for generation model
        if batchsize > 1:  # concatenate each input type if batchsize > 1
            label_maps = np.concatenate(list_label_maps, 0)
            means = np.concatenate(list_means, 0)
            stds = np.concatenate(list_stds, 0)
        else:
            label_maps = list_label_maps[0]
            means = list_means[0]
            stds = list_stds[0]
        
        # Convert to PyTorch tensors if requested
        if return_tensors:
            label_maps = torch.from_numpy(label_maps).to(device)
            means = torch.from_numpy(means).to(device)
            stds = torch.from_numpy(stds).to(device)
        
        # Return as dictionary for easier handling in PyTorch
        inputs = {
