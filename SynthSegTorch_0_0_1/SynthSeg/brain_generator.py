...
class BrainGenerator:
    ...
    def _build_labels_to_image_model(self):
        ...
        out_shape = lab_to_im_model.output_shape
        ...
        while True:
            ...
            for key, value in model_inputs.items():
                if isinstance(value, np.ndarray):
                    model_inputs_torch[key] = torch.from_numpy(value).to(self.device)
                else:
                    model_inputs_torch[key] = value
            
            # Forward pass through the model
            with torch.no_grad():
                # Extract required parameters from model_inputs_torch
                labels = model_inputs_torch['label_maps']
                means = model_inputs_torch['means']
                stds = model_inputs_torch['stds']
                # Pass them explicitly to the model
                image, labels = self.labels_to_image_model(labels, means, stds)
            
            # Convert back to numpy for consistency with the original API
            image_np = image.cpu().numpy() if isinstance(image, torch.Tensor) else image
            labels_np = labels.cpu().numpy() if isinstance(labels, torch.Tensor) else labels
            
            yield image_np, labels_np
